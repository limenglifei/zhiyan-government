(function () {
  ...
  function readReportRows() {
    ...
    return qa('tr', body).map((row, index) => {
      ...
      return { name: text(1) || text(0), region: text(2) || '长三角', honor: text(3) || '科技型中小企业', node, score: text(5) || text(4) || '90%', phone: text(6) || ('0512-6688-' + String(2201 + index)), email: text(7) || ('investment' + String(index + 1).padStart(2, '0') + '@enterprise.cn') };
    }).filter(Boolean);
  }
  ...
  function enhanceGapLinkage() {
    ...
    if (head) {
      head.innerHTML = '<th><input id="chainReportSelectAllV58" type="checkbox" aria-label="全选"></th><th>推荐企业</th><th>所在地区</th><th>企业荣誉资质</th><th>主要补强节点</th><th>匹配度</th><th>联系电话</th><th>企业邮箱</th>';
  ...
  function downloadChainReportV74() {
    const report = q('#chainReportV58');
    if (!report) return;
    const title = (q('#chainReportTitleV58')?.textContent || '产业链强链补链分析报告').trim();
    const content = '<!doctype html><html><head><meta charset="utf-8"><title>' + esc(title) + '</title><style>body{font-family:Microsoft YaHei,Arial;padding:36px;line-height:1.8;color:#17233d}h1{font-size:24px}pre{font:14px/1.9 Microsoft YaHei;white-space:pre-wrap}</style></head><body><h1>' + esc(title) + '</h1><pre>' + esc(report.innerText) + '</pre></body></html>';
    const url = URL.createObjectURL(new Blob([content], { type: 'application/msword;charset=utf-8' }));
    const link = document.createElement('a');
    link.href = url; link.download = title + '.doc'; document.body.appendChild(link); link.click(); link.remove();
    setTimeout(() => URL.revokeObjectURL(url), 500);
    if (typeof window.toast === 'function') window.toast('报告已开始下载');
  }
  function patchReportActionsV74() {
    q('#chainReportAllGapV72')?.remove();
    const actions = q('#chainReportV58 .report-actions-v58');
    if (!actions) return;
    qa('button', actions).forEach((button) => { if (button.textContent.includes('打印报告')) button.remove(); });
    const download = q('button', actions);
    if (download) { download.textContent = '↓ 下载报告'; download.className = 'primary'; download.onclick = downloadChainReportV74; }
  }
  function enhanceReportSoon() { const start = Date.now(); const timer = setInterval(() => { removeRiskSection(); enhanceGapLinkage(); patchReportActionsV74(); if (q('#chainReportRowsV58')?.dataset.v72 === '1' || Date.now() - start > 8000) clearInterval(timer); }, 250); }
  ...
  patchReportActionsV74();
  window.downloadChainReportV74 = downloadChainReportV74;
